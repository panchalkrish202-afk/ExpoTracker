...
def process_command(command):
    ...
    # Match "open [app]" - Case sensitive for app name
    open_match = re.match(r"^open\s+(.+)", command, re.IGNORECASE)
    if open_match:
        app_name = open_match.group(1).strip()
        return actions.open_app(app_name)

    # Match "close [app]" or "stop [app]" - Case sensitive for app name
    close_match = re.match(r"^(close|stop)\s+(.+)", command, re.IGNORECASE)
    if close_match:
        app_name = close_match.group(2).strip()
        return actions.close_app(app_name)

    # Match "search google for [query]" or "google [query]"
    google_match = re.match(r"^(search google for|google)\s+(.+)", command, re.IGNORECASE)
    if google_match:
        query = google_match.group(2).strip()
        return actions.google_search(query)

    # Match "ai search [query]" or "ask ai [query]"
    ai_match = re.match(r"^(ai search|ask ai)\s+(.+)", command, re.IGNORECASE)
    if ai_match:
